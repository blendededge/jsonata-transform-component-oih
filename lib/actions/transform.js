...
async function processAction(msg, cfg) {
  ...
  try {
    ...
    if (cfg.iso2to3) {
      this.logger.info('inside iso2to3');
      const { expression } = cfg;
      const compiledExpression = jsonata(expression);
      if (this && this.getFlowVariables) {
        compiledExpression.assign('getFlowVariables', () => this.getFlowVariables());
      }
      result = compiledExpression.evaluate(compatibleMessage.body);
    } else {
