...
async function processAction(msg, cfg) {
  ...
  function manualTransform(self) {
    ...
    compiledExpression.registerFunction('iso2to3', (Iso2) => lookup.byIso(Iso2));
    compiledExpression.registerFunction('phoneNumtoIso3', (phoneCode) => lookup.byIso(phoneCode));
    result = compiledExpression.evaluate(msg);
    self.logger.info(JSON.stringify(result));
    self.logger.info(JSON.stringify(lookup.byIso(826)));
    return result;
  }
  ...
  try {
    ...
    result = cfg.manualTransform ? manualTransform(this) : JsonataTransform.jsonataTransform(compatibleMessage, cfg, this);
  } catch (e) {
